...
) -> ServiceResult<(Vec<pb::record::Record>, Option<pb::paginate::Paginated>)> {
...
pub async fn query_list_by_user_id(
    conn: &mut PgConnection,
    user_id: Uuid,
) -> ServiceResult<Vec<pb::record::Record>> {
    let records = records::table
        .filter(records::user_id.eq(user_id))
        .get_results(conn)?;
    Ok(records)
}
pub async fn query_by_id(conn: &mut PgConnection, id: i32) -> ServiceResult<pb::record::Record> {
...
) -> ServiceResult<pb::record::Record> {
