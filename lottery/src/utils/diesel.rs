...
impl<T> Paginate for T {
    fn paginate(self, paginate: Option<pb::paginate::Paginate>) -> Paginated<Self> {
        let  paginate = paginate.unwrap_or(pb::paginate::Paginate{page:1,per_page: 10});
...
impl<T> Paginated<T> {
    ...
    pub fn load_and_paginated<'a, U>(self, conn: &mut PgConnection) -> QueryResult<(Vec<U>, Option<pb::paginate::Paginated>)>
    where
    ...
    {   
        ...
        Ok((records, Some(pb::paginate::Paginated{page, per_page, total, total_pages})))
    }
}
