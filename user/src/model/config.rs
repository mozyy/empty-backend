...
impl TryFrom<pb::user::auth::Pattern> for Pattern {
    type Error = Error;
    fn try_from(value: pb::user::auth::Pattern) -> Result<Self, Self::Error> {
        let pattern = value.pattern.ok_or_loss()?;
        let pattern = match pattern {
            pb::user::auth::pattern::Pattern::Equal(value) => Pattern::Equal(value),
            pb::user::auth::pattern::Pattern::Prefix(value) => Pattern::Prefix(value),
            pb::user::auth::pattern::Pattern::Regex(value) => {
...
impl TryFrom<pb::user::auth::Config> for Config {
    type Error = Error;
    fn try_from(value: pb::user::auth::Config) -> Result<Self, Self::Error> {
